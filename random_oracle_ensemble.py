...
# Cài đặt thuật toán Random Oracle 
class RandomOracleModel(object):
    ...
    def train(self, X, Y):
        ...
        # Chọn 2 điểm làm 2 tâm để chia dữ liệu huấn luyện 2 mô hình 
        len_train = len(X)
        ...
        # Chia dữ liệu ra 2 phần dựa vào khoảng cách với 2 điểm tâm 
        for x, y in zip(X, Y):
    ...
    # Dự đoán dựa vào 2 mô hình đã huấn luyện và 2 điểm tâm của 2 mô hình 
    def predict(self, x):

        assert self.model1 is not None and self.model2 is not None
        assert self.inst1 is not None and self.inst2 is not None
        
        # Chọn mô hình dự đoán dựa và khoảng cách giữa 2 điểm tâm 
...
# Cai đặt thuật toán Adaboost kết hợp với adaboost
class SmoteAdaboost(object):
    ...
    # Huấn luyện mô hình Smote Adaboost 
    def fit(self, X, Y):

        smote = SMOTE(random_state=42)
        X_sm, Y_sm = smote.fit_sample(X, Y) # Dùng smote để xử lý dữ liệu không cân bằng
        self.adaboost_model.fit(X_sm, Y_sm)
    
    # Hàm dự đoán cho Smote Adaboost 
    def predict(self, x):
...
def test_smoteadaboost():
    ...
    # Try with Oracle Ensemble
    # Sử dụng mô hình kết hợp với Random Oracle 
    random_oracle_model = RandomOracleModel(base_learning="smoteboost")
    random_oracle_model.train(train_x, train_y)
    print("Accuracy Score for Oracle ensemble: \n", random_oracle_model.evaluate(test_x, test_y))

    # Try Save and Load module
    # Sử dụng chức năng saving và loading 
    random_oracle_model.save("model.pkl")
